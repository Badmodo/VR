...
namespace BreadAndButter.VR
{
    ...
    public class Pointer : MonoBehaviour
    {
        ...
        void Update()
        {
            //If we aren't active, we don't need to run the method
            if (!Active)
                return;
        ...
        private void UpdateScalePos(RaycastHit _hit, bool _didHit)
        {
            if (_didHit)
            {
                CalculateDirAndDst(controller.transform.position, _hit.point, out _, out float distance);

                //set the tracer position to the midpoint of the parent pos and the Tndpoint
            ...
            else
            {
                ...
                CalculateDirAndDst(controller.transform.position,
                    controller.transform.position + controller.transform.forward * 100.0f,
                    out Vector3 dir,
                    out float distance);
                ...
                cursor.position = controller.transform.position + controller.transform.forward * 100.0f;
                cursor.localScale = Vector3.one * cursorScaleFactor;
            }
        }
